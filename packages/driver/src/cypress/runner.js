...
const RUNNABLE_PROPS = 'id title root hookName hookId err state failedFromHookId body speed type duration wallClockStartedAt wallClockDuration timings attemptIndex final'.split(' ')
    ...
    switch (name) {
      case 'afterEach': {
        ...
        if (t) {
          ...
          console.log('willRetry:', willRetry)
          ...
          if (this.suite.root && ((t_1 !== _.last(allTests)) && (t_1 !== _.last(tests)) || willRetry)) {
            console.log('AE: change to after')
      ...
      case 'afterAll': {
        ...
        if (t) {
          ...
          if (
            ...
            console.log('AA: change to after')
            changeFnToRunAfterHooks()
          }
        }
...
const create = function (specWindow, mocha, Cypress, cy) {
  ...
  mocha.onRunnableRun(function (run, runnable, args) {
    ...
    test.prevAttempts = test.prevAttempts || []

    if (test._retries === -1) {
      test._retries = getDefaultRetries(Cypress)
    }

    const canRetry = test.attemptIndex < test._retries
    ...
    if (test.attemptIndex > 0 && _testsById[test.id] !== test) {
      ...
      _.keys(testRef).forEach((key) => {
        return delete testRef[key]
      })

      _.assign(testRef, test)

      testRef.prevAttempts = prevAttemptClone.prevAttempts.concat([prevAttemptClone])
  ...
  const onNextError = (runnable, next, err) => {
    ...
    if (err) {
      ...
      const canRetry = test._currentRetry < test._retries

      if (canRetry) {
        test.state = 'failed'
      }

      if (isBeforeHook && canRetry) {
        test.trueFn = test.fn
        test.fn = function () {
          throw err
        }
    ...
    getTestsState () {
      ...
      for (let test of _tests) {
        if (test.id === id) {
          ...
          wrappedAttemps.forEach(mixinLogs)
    ...
    getErrorByTestId (testId, attemptIndex) {
      ...
      if (test) {
        if (attemptIndex != null) {
          test = test.attempts[attemptIndex]
        }
...
const getAllAttemptsFromTest = (test) => {
  const lastAttempt = wrapAll(test)
  const attempts = test.prevAttempts.concat([lastAttempt])

  return attempts
}

const mixinLogs = (attempt) => {
  _.each(RUNNABLE_LOGS, (type) => {
    const logs = attempt[type]
